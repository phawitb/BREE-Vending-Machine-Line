...
def profile(vm_id):
    ...
    if data.get('userId'):
        ...
        if vm_doc and 'admin' in vm_doc:
            is_admin = data['userId'] in vm_doc['admin']

    session['point'] = point
    session['is_admin'] = is_admin
    return jsonify({"status": "ok", "point": point, "is_admin": is_admin})
...
def view_profile(vm_id):
    ...
    return render_template('profile.html', line_name=line_name, user_picture=user_picture, vm_id=vm_id, point=point, show_manage_button=show_manage_button)
...
def add_product(vm_id):
    ...
    if vm_doc:
        ...
        if any(p['barcode'].strip().lower() == barcode.lower() for p in products):
            flash(f"❌ barcode : '{barcode}' มีอยู่แล้ว ไม่สามารถเพิ่มซ้ำได้", 'danger')
...
@app.route('/<vm_id>/api/save_order', methods=['POST'])
def save_order(vm_id):
    try:
        data = request.get_json()
        ...
        vm_doc = vms_collection.find_one({'vmId': vm_id})
        if not vm_doc or 'products' not in vm_doc:
            return jsonify({'status': 'error', 'message': 'VM not found'}), 404

        products = vm_doc['products']
        id_to_product = {p['id']: p for p in products}

        reordered_products = []
        for pid in new_order:
            if pid in id_to_product:
                reordered_products.append(id_to_product[pid])

        # ถ้าเรียงใหม่ครบทุกตัว ก็ update
        if len(reordered_products) == len(products):
            vms_collection.update_one({'vmId': vm_id}, {'$set': {'products': reordered_products}})
            return jsonify({'status': 'success'})
        else:
            return jsonify({'status': 'error', 'message': 'Order length mismatch'}), 400
